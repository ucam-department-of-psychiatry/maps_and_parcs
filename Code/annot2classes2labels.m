function [classes labels] = annot2classes2labels(lh_annot, rh_annot, hemi_split)
...
for ii = 1:length(lh_l)
    ...
    elseif isempty(which_row)
        classes(ii) = 0;
        labels{ii} = "no matching lookup";
    else
        classes(ii) = which_row;
        labels{ii} = lh_ctb.struct_names{which_row};
    end
end
for ii = 1:length(rh_l)
    ...
    if rh_l(ii) == 0
        ...
        labels{ii+length(lh_l)} = "no label";
    elseif isempty(which_row)
        classes(ii+length(lh_l)) = 0;
        labels{ii+length(lh_l)} = "no matching lookup";
    else
        if hemi_split == 1
            classes(ii+length(lh_l)) = which_row + length(lh_ctb.table(:,5));
            labels{ii+length(lh_l)} = rh_ctb.struct_names{which_row};
        else
